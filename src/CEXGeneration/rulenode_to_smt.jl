...
function _rulenode_to_smt_impl(
    ...
    # Typed nonterminal wrappers generated by @csgrammar.
    # Examples: Expr = BoolExpr, BoolExpr = (...)
    if op_name == :Expr
        length(node.children) >= 1 || error("Expr wrapper requires at least 1 child")
        return _rulenode_to_smt_impl(node.children[1], grammar, expected_type)
    elseif op_name == :BoolExpr
        length(node.children) >= 1 || error("BoolExpr wrapper requires at least 1 child")
        bool_smt = _rulenode_to_smt_impl(node.children[1], grammar, :bool)
        return expected_type == :int ? "(ite $bool_smt 1 0)" : bool_smt
    end
